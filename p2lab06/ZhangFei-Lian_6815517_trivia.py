'''
Trivia competition game By Fei-Lian and Jaden
'''
import requests
import html
import random
...
def instructions()-> int:
    ...
    #giving a choice to play the game 1 or 1v1 and makes sure that it gets a valid answer
    while True:
        try:
...
def get_questions(link: str)-> dict:
    '''uses a link provided and accesses and returns the dictionary from the API database'''
...
def whos_turn(questionsasked: int,p1wrong:int, p2wrong: int)-> int:
    ''' takes in how many questions have been asked and returns whos turn it is as 1 or 2'''
    #the loop starts at 0 so when there is odd number of questions asked ie.1 then ot is p2's turn
...
def ask_question(quizbank: dict, num: int)-> str: 
    ...
    if (question["type"]) == "multiple":
        ...
        #matching the answer abcd to the actual word answer 
        ...
        #checking the input's syntax so capitalization doesn't matter and t andd f work too
        while True:
...
def main():
    ...
    #calling the instructions function to save the "amount" of players playing
    player_amount = instructions()

    #for loop here instead of while loop because the variable i is useful for acessing questions in the questions list
    for i in range(100):
...
#actual gameplay
        #printing out how many the player who is going has gotten wrong
        if player_amount == 1:
